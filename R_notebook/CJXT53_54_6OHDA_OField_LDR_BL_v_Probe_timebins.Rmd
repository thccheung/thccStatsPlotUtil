---
title: "CJXT53-54 6OHDA OField LDR, DLC output analysis, BL vs. LDR Probe, timebins"
author: "THC Cheung"
...
for (i in 1:length(my_colnames_non_factor)) {
    ...
    if (exclude_cw_ccw & contains_cw_ccw) {
        ...
        my_plot_1 <- quick_lineplot_mean_sem_indi(
            data_plot, start_time_in_min, var, sess_name, MouseID) +
            xlab("time") +
            theme(axis.title.y = element_blank()) +
            ggtitle(my_var)
        my_plot_1 <- my_plot_1 +
            scale_colour_manual(values=my_colour_set[c(1, 2)]) +
            scale_fill_manual(values=my_colour_set[c(1, 2)]) +
            my_ylim

        data_plot <- data_test %>%
            filter(context == "unpaired")

        my_plot_2 <- quick_lineplot_mean_sem_indi(
            data_plot, start_time_in_min, var, sess_name, MouseID) +
            xlab("time") +
            theme(axis.title.y = element_blank()) +
            ggtitle(my_var)
        my_plot_2 <- my_plot_2 +
            scale_colour_manual(values=my_colour_set[c(3, 4)]) +
            scale_fill_manual(values=my_colour_set[c(3, 4)]) +
            my_ylim
